...
{ 
	...
	int dims = src.channels(); //行数为src的像素点数，列数为通道数，每列数据分别为src的bgr，从上到下 从左到右顺序读数据 
	...
	for (int row = 0; row < height1; row++) 
	{
		...
		{ 
			...
			{//因为高斯模糊的关系，所以mask元素的颜色除了黑白色还有黑白边缘经过模糊后的非黑白值 
				w = m / 255.0; 
